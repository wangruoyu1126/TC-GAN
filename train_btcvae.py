...
if(params['dataset'] == 'MNIST'):
    ...
    from models.dsprite_model_modified import Generator, Discriminator, DHead, QHead
...
for epoch in range(params['num_epochs']):
    ...
    for i, (data, _) in enumerate(dataloader, 0):
        ...
        # print('q_mu', q_mu)
        # print('q_var', q_var)
        # torch.save(q_mu, 'q_mu.pt')
        # torch.save(q_var, 'q_var.pt')

        # print('q_mu.mean()', q_mu.mean())
        # print('q_var.mean()', q_var.mean())

        logvar = torch.log(q_var)
        latent_dist = (q_mu, logvar)
        latent_sample = reparametrize(q_mu, logvar)
        # print('latent_sample', latent_sample)
        # torch.save(latent_sample, 'latent_sample.pt')

        _, log_qz, log_prod_qzi, _ = get_log_pz_qz_prodzi_qzCx(latent_sample,
                                                               latent_dist,
                                                               n_data=737280,
                                                               is_mss=False)
        # torch.save(log_qz, 'log_qz.pt')
        # torch.save(log_prod_qzi, 'log_prod_qzi.pt')
        # print('log_qz, log_prod_qzi', log_qz, log_prod_qzi)
        tc_loss = (log_qz - log_prod_qzi).mean()
        # print('tc_loss', tc_loss)
        ...
        # G_loss_org = gen_loss + dis_loss + con_loss
        G_loss = gen_loss + dis_loss + args.info_weight * con_loss + args.tc_weight * tc_loss
        print('gen_loss: {}, con_loss: {}, tc_loss: {}, G_loss: {}'.format(gen_loss, con_loss, tc_loss, G_loss))
